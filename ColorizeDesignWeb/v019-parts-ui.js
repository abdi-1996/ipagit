// v0.19.3 — UI bridge for the separate-parts renderer.
// Keeps the controls clear of the iPhone bottom dock and safe area.
...
function init(){
  ...
  if(!document.getElementById('partsPanel19Style')){
    const style=document.createElement('style');style.id='partsPanel19Style';style.textContent=`#partsPanel19{position:absolute;left:12px;right:12px;bottom:56px;z-index:40;display:flex;gap:6px;align-items:center;justify-content:center;padding:7px 8px;border-radius:12px;background:rgba(20,22,26,.82);backdrop-filter:blur(16px);-webkit-backdrop-filter:blur(16px);box-shadow:0 8px 24px rgba(0,0,0,.26);font:12px -apple-system,BlinkMacSystemFont,sans-serif;color:#fff}#partsPanel19 button,#partsPanel19 select{height:30px;border:1px solid rgba(255,255,255,.15);border-radius:8px;background:rgba(255,255,255,.09);color:#fff;padding:0 9px;font:inherit}#partsPanel19 button.off{opacity:.4;text-decoration:line-through}#partsPanel19 select option{color:#111;background:#fff}@media(max-width:720px){#partsPanel19{left:10px;right:10px;bottom:calc(96px + env(safe-area-inset-bottom));justify-content:center;flex-wrap:wrap}}`;
...
globalThis.__colorizePartsUI19Debug=()=>({version:'0.19.3',...state,panel:!!document.getElementById('partsPanel19')});
