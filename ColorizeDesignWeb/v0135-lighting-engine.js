...
let debugState={frames:0,owns:false,version:'0.13.5',directUpdates:0};
let drawingNow=false;

function deepMerge(extra){
  const s=extra||{};
  return {...DEFAULTS,...s,main:{...DEFAULTS.main,...(s.main||{})},hdr:{...DEFAULTS.hdr,...(s.hdr||{})}};
}
function readSettings(){
  if(liveSettings)return deepMerge(liveSettings);
  try{return deepMerge(JSON.parse(localStorage.getItem(SETTINGS_KEY)||'{}'))}catch{return structuredClone(DEFAULTS)}
}
...
function isLegacyWarmFill(p){return !!(p?.isPointLight&&p.color?.getHex?.()===0xffd5b0&&Number(p.distance)>=30&&Number(p.intensity)>=8)}
...
function deriveDominant(texture){
  ...
  for(let y=step;y<h-step;y+=step){
    ...
    for(let x=0;x<w;x+=step){
      const i=(y*w+x)*channels,r=Number(data[i])||0,g=Number(data[i+1])||0,b=Number(data[i+2])||0;
      const score=(.2126*r+.7152*g+.0722*b)*solid;
      if(score>best){best=score;bx=x;by=y}
    }
  }
  const u=bx/Math.max(1,w-1),v=by/Math.max(1,h-1),theta=u*Math.PI*2-Math.PI,phi=v*Math.PI;
  const dir=new THREE.Vector3(Math.sin(phi)*Math.sin(theta),Math.cos(phi),Math.sin(phi)*Math.cos(theta));
  if(dir.z<.08)dir.z=Math.abs(dir.z)+.12;
...
async function buildEnvironment(renderer,preset,token){
  if(preset==='studio'){
    ...
    const room=new RoomEnvironment();pmremBusy=true;let target;
    try{const pmrem=new THREE.PMREMGenerator(renderer);target=pmrem.fromScene(room,.04);pmrem.dispose()}
    finally{pmremBusy=false;room.dispose?.()}
    return {target,source:null,dominant:new THREE.Vector3(-.5,.68,.54).normalize(),token};
  }
  const source=await new RGBELoader().loadAsync(HDRS[preset]||HDRS.dayStreet);
  source.mapping=THREE.EquirectangularReflectionMapping;
  const dominant=deriveDominant(source);pmremBusy=true;let target;
  try{const pmrem=new THREE.PMREMGenerator(renderer);pmrem.compileEquirectangularShader();target=pmrem.fromEquirectangular(source);pmrem.dispose()}
...
function ensureEnvironment(renderer,scene,s){
  ...
  const preset=s.hdr.preset||'dayStreet',identity=preset==='custom'?'custom':preset;
  if(identity==='custom')return;
  if(envIdentity===identity&&envTarget){if(scene.environment!==envTarget.texture)scene.environment=envTarget.texture;return}
  if(envPromise?.identity===identity)return;
  const token=++envToken,promise=buildEnvironment(renderer,identity,token);envPromise={identity,promise};
  promise.then(next=>{
    if(token!==envToken){next.target?.dispose?.();next.source?.dispose?.();return}
    disposeEnvironment();envTarget=next.target;envSource=next.source;envIdentity=identity;hdrDominant=next.dominant||hdrDominant;
    if(editorScene===scene&&readSettings().mode==='hdr'){scene.environment=envTarget.texture;drawNow()}
  }).catch(err=>{debugState.envError=String(err?.message||err)}).finally(()=>{if(envPromise?.promise===promise)envPromise=null});
}
...
function ensureKeyTarget(scene,key){
  if(!key?.target)return;
  if(!key.target.parent){
    key.target.userData.colorizeLightTarget=true;
...
function applyLighting(renderer,scene,camera,s){
  const {dirs,hems,points,ambients}=collectLights(scene),key=dirs[0];
  ...
  if(key){
    ...
    if(key.position.z<1.2)key.position.z=1.2;
    ...
    key.updateMatrixWorld(true);key.shadow.camera?.updateMatrixWorld?.(true);
  }
  for(const h of hems){
    h.visible=s.mode==='main';
    if(s.mode==='main'){h.intensity=clamp(s.main.ambient,0,3);h.color.set(0xffffff);h.groundColor.set(0x343844)}
  }
  renderer.toneMapping=THREE.ACESFilmicToneMapping;
  renderer.toneMappingExposure=s.mode==='hdr'?clamp(s.hdr.exposure,.2,3):clamp(s.main.exposure,.2,3);
  renderer.shadowMap.enabled=true;renderer.shadowMap.autoUpdate=true;renderer.shadowMap.needsUpdate=true;
  ...
  debugState={
    ...debugState,version:'0.13.5',frames:(debugState.frames||0)+1,owns:true,mode:s.mode,
    keyIntensity:key?.intensity??null,keyPosition:key?key.position.toArray():null,
    ambient:hems.find(h=>h.visible)?.intensity??0,exposure:renderer.toneMappingExposure,
    envIntensity:scene.environmentIntensity??0,warmFillVisible:points.filter(isLegacyWarmFill).some(p=>p.visible),
    canvasParent:renderer.domElement?.parentElement?.id||renderer.domElement?.parentElement?.className||'',
    sceneLights:{directional:dirs.length,hemisphere:hems.length,point:points.length},at:performance.now()
  };
}

function drawNow(){
  if(!editorRenderer||!editorScene||!editorCamera||drawingNow||pmremBusy)return false;
  drawingNow=true;
  try{
    applyLighting(editorRenderer,editorScene,editorCamera,readSettings());
    editorRenderer.render(editorScene,editorCamera);
    return true;
  }catch(err){debugState.directDrawError=String(err?.stack||err);return false}
  finally{drawingNow=false}
}

// The wrapped renderer calls this immediately before every real editor frame.
globalThis.__colorizeBeforeThreeRender=(renderer,scene,camera)=>{
  ...
  if(!owns){debugState.nonEditorFrames=(debugState.nonEditorFrames||0)+1;return}
  editorRenderer=renderer;editorScene=scene;editorCamera=camera;
  applyLighting(renderer,scene,camera,readSettings());
};

// Direct bridge used by the slider UI. This avoids waiting for localStorage, rAF,
// or Safari's pointer-event scheduling while a finger is dragging a range control.
globalThis.__colorizeLightingSetSettings=(next)=>{
  liveSettings=deepMerge(next);
  try{localStorage.setItem(SETTINGS_KEY,JSON.stringify(liveSettings))}catch{}
  debugState.directUpdates=(debugState.directUpdates||0)+1;
  if(editorRenderer&&editorScene&&editorCamera){
    applyLighting(editorRenderer,editorScene,editorCamera,liveSettings);
    drawNow();
    requestAnimationFrame(drawNow);
  }
  return true;
};
globalThis.__colorizeLightingUseStored=()=>{liveSettings=null;drawNow()};
globalThis.__colorizeLightingDebug=()=>({...debugState,hookError:globalThis.__colorizeLightingHookError||null});

window.addEventListener('DOMContentLoaded',()=>{
  document.addEventListener('visibilitychange',()=>{if(!document.hidden)setTimeout(drawNow,0)});
  window.addEventListener('pageshow',()=>setTimeout(drawNow,0));
  window.addEventListener('resize',()=>requestAnimationFrame(drawNow),{passive:true});
});
