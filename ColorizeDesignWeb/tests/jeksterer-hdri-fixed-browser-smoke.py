...
try:
    ...
    result=evaluate("""(async()=>{const w=t=>new Promise(r=>setTimeout(r,t));for(let i=0;i<220;i++){if(document.readyState==='complete'&&typeof window.__colorizeHdri251Debug==='function')break;await w(100)}document.querySelector('.mode[data-mode="view3d"]')?.click();document.getElementById('lightingBtn')?.click();for(let i=0;i<220;i++){if(document.querySelector('[data-light-mode="hdr"]'))break;await w(100)}document.querySelector('[data-light-mode="hdr"]')?.click();const ok=await window.__colorizeHdri251UseBuiltin?.();for(let i=0;i<240;i++){const d=window.__colorizeHdri251Debug?.();if(ok&&d?.loaded&&d.width===64&&d.height===32)return {ok,debug:d};await w(100)}return {ok,debug:window.__colorizeHdri251Debug?.()||null}})()""",True)
    print('V0251 BUILTIN',json.dumps(result,ensure_ascii=False))
    d=(result or {}).get('debug') or {}
    if not (result or {}).get('ok') or d.get('version')!='0.25.1' or not d.get('loaded'): raise RuntimeError('Fixed built-in HDRI did not load: '+json.dumps(d))
    if int(d.get('width') or 0)!=64 or int(d.get('height') or 0)!=32: raise RuntimeError('Unexpected fixed HDRI dimensions')
    custom=evaluate("""(async()=>{const w=t=>new Promise(r=>setTimeout(r,t));const ab=await window.__colorizeHdri251Buffer?.();const file=new File([ab],'iphone-import-test.hdr',{type:'image/vnd.radiance'});const ok=await window.__colorizeHdri251Import?.(file);for(let i=0;i<240;i++){const d=window.__colorizeHdri251Debug?.();if(ok&&d?.loaded&&d.kind==='custom'&&d.imports>0)return {ok,debug:d};await w(100)}return {ok,debug:window.__colorizeHdri251Debug?.()||null}})()""",True)
    print('V0251 CUSTOM',json.dumps(custom,ensure_ascii=False))
    cd=(custom or {}).get('debug') or {}
    if not (custom or {}).get('ok') or cd.get('kind')!='custom' or int(cd.get('imports') or 0)<1: raise RuntimeError('Fixed custom import failed: '+json.dumps(cd))
    print('jeksterer-hdri-fixed-browser-smoke: PASS — built-in Jeksterer HDRI parses and iOS-safe custom import works')
finally:
    try:
        if ws: ws.close()
    except: pass
