...
profile=tempfile.mkdtemp(prefix='colorize-v032-'); log=open('/tmp/colorize-v032-text-chrome.log','w+b')
...
try:
    ...
    url=f'http://127.0.0.1:{PORT}/?v032-stability=1'
    ...
    ws=websocket.create_connection(page['webSocketDebuggerUrl'],timeout=8); cid=0
    ...
    for _ in range(100):
        try:
            ready=bool(evaluate("(window.__colorizeV032LoadState==='ready'||window.__colorizeV031LoadState==='ready') && window.__colorizeText31Debug?.().version==='0.31.0' && window.__colorizeTools31Debug?.().toolCount>=24"))
            if ready: break
        except Exception: pass
        time.sleep(.15)
    if not ready:
        err=evaluate("window.__colorizeV032LoadError||window.__colorizeV031LoadError||window.__colorizeV030LoadError||'not ready'")
        raise RuntimeError('current editor did not become ready: '+str(err))
    ...
    evaluate("document.getElementById('colorizeTextEditor31').value='COLORIZE STABLE 32'; true")
    time.sleep(2.1)
    ...
    print('V032 STABILITY',json.dumps({'before':before,'editing':editing,'stable':stable,'saved':saved,'vector':vec},ensure_ascii=False),flush=True)
    print('editor-stability: PASS — text stays visible and vector properties persist',flush=True)
finally:
    try:
        if ws: ws.close()
    except: pass
