...
try:
    ...
    clean=evaluate("""(async()=>{const w=t=>new Promise(r=>setTimeout(r,t));for(let i=0;i<220;i++){if(typeof window.__colorizeClean3D21Debug==='function'&&document.querySelector('.mode[data-mode="view3d"]'))break;await w(100)}window.__colorizeSetFacade21(false,'test-start');document.querySelector('.mode[data-mode="view3d"]')?.click();for(let i=0;i<300;i++){const d=window.__colorizeClean3D21Debug?.();if(d?.version==='0.21.0'&&d.gridHidden&&d.wallsDetected>0&&d.wallsVisible===0&&d.floorsVisible===0)return d;await w(100)}return window.__colorizeClean3D21Debug?.()||null})()""",True)
