...
let contourDrag=null,originDrag=null,lastArea=null;
...
function previewContour(n,w,o){const fake={...o,contourWidth:w};contourLayer(n,fake)}
function startContour(e){if(tool()!=='contour'||read()?.mode!=='2d'||$('#colorizeTextEditor31'))return;const n=e.target?.closest?.('#world .obj.vector.selected[data-object-id]');if(!n)return;const {o}=activeData();if(!o||o.id!==n.dataset.objectId)return;e.preventDefault();e.stopImmediatePropagation();contourDrag={id:o.id,node:n,pointer:e.pointerId,startX:e.clientX,start:Number(o.contourWidth)||0,o};n.setPointerCapture?.(e.pointerId);toast('Контур: тяните вправо/влево',900)}
...
function decorateOriginAny(){const {o}=activeData();$$('.v032-origin').forEach(x=>x.remove());if(tool()!=='point'||!o||o.type==='vector')return;const n=node(o.id);if(!n)return;n.style.transformOrigin=`${clamp(o.originX??.5,0,1)*100}% ${clamp(o.originY??.5,0,1)*100}%`;const h=document.createElement('button');h.className='v032-origin';h.title='Центр трансформации';h.style.left=`${clamp(o.originX??.5,0,1)*100}%`;h.style.top=`${clamp(o.originY??.5,0,1)*100}%`;n.appendChild(h)}
function startOrigin(e){const h=e.target?.closest?.('.v032-origin');if(!h||tool()!=='point')return;const n=h.closest('.obj[data-object-id]');if(!n)return;e.preventDefault();e.stopImmediatePropagation();originDrag={id:n.dataset.objectId,node:n,h,pointer:e.pointerId};h.setPointerCapture?.(e.pointerId)}
...
window.addEventListener('pointerdown',e=>{if(startOrigin(e)!==undefined&&originDrag)return;if(tool()==='area'&&areaAt(e)){e.preventDefault();e.stopImmediatePropagation();return}startContour(e)},true);
...
function decorate(){decorateContours();decorateOriginAny()}
decorate();new MutationObserver(()=>requestAnimationFrame(decorate)).observe($('#world')||document.body,{childList:true,subtree:true});setInterval(decorate,750);
